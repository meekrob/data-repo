#!/usr/bin/env bash
# Template for testing datadir.sh
set -euo pipefail
...
EXPECTED_PREFIX="file1"   # adjust per test
# - Expected files inside destination dir
EXPECTED_FILES=("file1.txt" "file2.txt")
# - Should original files remain? (true/false)
...
# ===== Verify Results =====
# Find the destination directory (timestamped)
...
for f in "${EXPECTED_FILES[@]}"; do
    if $ORIGINAL_SHOULD_EXIST && [[ ! -f "$f" ]]; then
        ...
        echo "FAIL: original file $f should have been moved"
        exit 1
    fi
done
